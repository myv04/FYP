...
from course_data import web_systems_students

web_systems_dashboard = Blueprint("web_systems_dashboard", __name__)

def init_dashboard(flask_app):
    dash_app = dash.Dash(
        name="web_systems_dashboard",
        server=flask_app,
        url_base_pathname="/dashboard_web_systems/",
        suppress_callback_exceptions=True
    )

    df = pd.DataFrame(web_systems_students)
    ...
    df["Assignment 1 (Web Tech)"] = pd.to_numeric(df["a1_score"], errors="coerce")
    ...
    df.loc[dupes, "StudentLabel"] = df.loc[dupes].apply(
        lambda row: f"{row['Student']} ({str(row.get('ID', ''))[-3:]})", axis=1
    )

    column_order = [
        "ID", "Student", "Final Grade", "Attendance (%)",
        "Assignment 1 (Web Tech)", "Assignment 1 Status", "Assignment 1 Penalty", "Status"
    ]

    df = df[column_order + ["StudentLabel"]]

    dash_app.layout = html.Div(children=[
        html.H1("🌐 Web Systems - Performance & Grades", style={"textAlign": "center"}),
        ...
        dcc.Dropdown(
            ...
            options=[{"label": "Assignment 1 (Web Tech)", "value": "Assignment 1 (Web Tech)"}],
            value="Assignment 1 (Web Tech)",
            clearable=False
        ),
...
def get_web_systems_attendance():
    try:
        df = pd.DataFrame(web_systems_students)
        df["attendance"] = pd.to_numeric(df["attendance"], errors="coerce")
