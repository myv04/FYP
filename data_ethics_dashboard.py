...
data_ethics_dashboard = Blueprint("data_ethics_dashboard", __name__)

def init_data_ethics_dashboard(flask_app):
    dash_app = dash.Dash(
        name="data_ethics_dashboard",
        server=flask_app,
        url_base_pathname="/data_ethics_dashboard/",
    ...
    df["Assignment 1 (Bias and Fairness Audits)"] = df["a1_score"]
    df["Assignment 1 Status"] = df["a1_status"]
    df["Assignment 1 Penalty"] = df["a1_penalty"]
    df["Assignment 2 (Privacy and Consent Policies)"] = df["a2_score"]
    ...
    column_order = [
        ...
        "Assignment 1 (Bias and Fairness Audits)", "Assignment 1 Status", "Assignment 1 Penalty",
        "Assignment 2 (Privacy and Consent Policies)", "Assignment 2 Status", "Assignment 2 Penalty", "Status"
    ]
    df = df[column_order]

    dash_app.layout = html.Div(children=[
        html.H1("⚖️ Data Ethics & Governance Dashboard (DS204)", style={"textAlign": "center"}),
        ...
        dcc.Dropdown(
            ...
            options=[
                {"label": "Assignment 1 (Bias and Fairness Audits)", "value": "Assignment 1 (Bias and Fairness Audits)"},
                {"label": "Assignment 2 (Privacy and Consent Policies)", "value": "Assignment 2 (Privacy and Consent Policies)"}
            ],
            value="Assignment 1 (Bias and Fairness Audits)", clearable=False
        ),
...
from course_data import data_science_students  # or use a dedicated list if you have one

def get_data_ethics_attendance():
    df = pd.DataFrame(data_science_students)
    df["attendance"] = pd.to_numeric(df["attendance"], errors="coerce")
    return round(df["attendance"].mean(), 2)

def get_data_ethics_grade():
    def calc(row):
        try:
