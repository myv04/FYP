...
software_testing_dashboard = Blueprint("software_testing_dashboard", __name__)

def init_software_testing_dashboard(flask_app):
    dash_app = dash.Dash(
        name="software_testing_dashboard",
        server=flask_app,
        url_base_pathname="/software_testing_dashboard/",
    ...
    df["Assignment 1 (Unit Testing Strategies)"] = df["a1_score"]
    df["Assignment 1 Status"] = df["a1_status"]
    df["Assignment 1 Penalty"] = df["a1_penalty"]
    df["Assignment 2 (Test Automation Frameworks)"] = df["a2_score"]
    ...
    column_order = [
        ...
        "Assignment 1 (Unit Testing Strategies)", "Assignment 1 Status", "Assignment 1 Penalty",
        "Assignment 2 (Test Automation Frameworks)", "Assignment 2 Status", "Assignment 2 Penalty", "Status"
    ]
    df = df[column_order]

    dash_app.layout = html.Div(children=[
        html.H1("🧪 Software Testing & QA Dashboard (SE203)", style={"textAlign": "center"}),
        ...
        dcc.Dropdown(
            ...
            options=[
                {"label": "Assignment 1 (Unit Testing Strategies)", "value": "Assignment 1 (Unit Testing Strategies)"},
                {"label": "Assignment 2 (Test Automation Frameworks)", "value": "Assignment 2 (Test Automation Frameworks)"}
            ],
            value="Assignment 1 (Unit Testing Strategies)", clearable=False
        ),
...
from course_data import software_engineering_students  # or your actual SE list

def get_software_testing_attendance():
    df = pd.DataFrame(software_engineering_students)
    df["attendance"] = pd.to_numeric(df["attendance"], errors="coerce")
    return round(df["attendance"].mean(), 2)

def get_software_testing_grade():
    def calc(row):
        try:
